...
## 🌟 v1.2 重大更新
因應現場實務需求，本版本新增以下功能：
1.  **多單位支援**：新增「台分」單位選擇，系統自動換算為公分計算，並於報表中保留原始輸入數值。
2.  **容許誤差設定 (Tolerance)**：可自定義棧板長寬與高度的「允許超出範圍 (Overhang)」，以符合節省棧板的現場實務。
3.  **預設值優化**：預設帶入常用之台分尺寸範例，提升輸入效率。
...
5.  **Excel 匯出**：一鍵產出包含匯總（含單位換算對照）與詳細堆疊清單的 .xlsx 報表。
...
## 📝 版本紀錄 (Changelog)
- **v1.2 (Current)**: 新增台分/公分單位切換、新增棧板容許超出範圍 (Overhang) 設定、優化預設值。
- **v1.1**: 基礎單位換算功能試行。
